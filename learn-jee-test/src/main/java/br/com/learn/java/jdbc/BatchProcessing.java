package br.com.learn.java.jdbc;
	...
	public void createTables() {
		try {
			connection.createStatement()
					.executeUpdate("create table EMPLOYEE (ID VARCHAR(36), NAME VARCHAR(45), DESIGNATION VARCHAR(15))");
			connection.createStatement().executeUpdate(
					"create table EMP_ADDRESS (ID VARCHAR(36), EMP_ID VARCHAR(36), ADDRESS VARCHAR(45))");
	...
	public void useStatement() {
		try {
			...
			for (int i = 0; i < EMPLOYEES.length; i++) {
				...
				statement.addBatch(
						String.format(insertEmployeeAddrSQL, UUID.randomUUID().toString(), employeeId, ADDRESSES[i]));
			}
